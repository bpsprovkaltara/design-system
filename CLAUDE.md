...
Panduan ini berlaku untuk Claude Code pada seluruh repo `bpskaltara-design-system`. Gunakan panduan ini untuk bekerja dari source aktif, menjaga kontrak consumer, dan memisahkan arsitektur repo dari framework aplikasi yang mengonsumsi paket.

## Ringkasan proyek

Repo ini memuat sistem desain internal BPS Provinsi Kalimantan Utara versi paket `4.6.0`. Satu source tree mempunyai dua mode:
...
Repo ini **bukan aplikasi Next.js**. Directive `'use client'` pada modul tertentu hanya menjaga kompatibilitas konsumen React Server Components, termasuk konsumen Next.js. Directive itu bukan aturan framework repo. `vite.lib.config.ts` memakai `preserveModules` agar directive tetap tersedia pada output subpath.
...
`design/DESIGN.md` adalah referensi historis Kinara, bukan panduan aktif BPS Kaltara. `dist/` adalah output terbangun, bukan tempat mengedit source. Dokumen dapat tertinggal dari source; verifikasi identifier, path, versi, dan contoh sebelum menggunakannya.
...
`pnpm build` membangun showcase melalui `tsc && vite build`. `pnpm build:lib` menjalankan konfigurasi library lalu menyalin `tokens.css` dan `fonts.css` ke `dist/`. Pilih pemeriksaan sesuai matriks pengujian; jangan menjalankan seluruh suite bila perubahan dan risikonya tidak memerlukan itu.
...
5. `scripts/copy-css-assets.mjs` menyalin CSS sumber yang diekspor terpisah
6. `package.json#exports` menentukan subpath yang didukung
...
Jaga ekspor berikut tetap sinkron dengan `package.json`, `src/index.ts`, dan hasil library build:
...
Secara fisik, `tokens.css` menempatkan alias shadcn pada blok Layer B dan menyatakan token khusus komponen lama sudah dihapus. Jangan menghidupkan kembali `--button-*`, `--input-*`, atau token komponen tanpa kebutuhan nyata dan wiring lengkap. Komponen memakai token semantik atau alias kompatibilitas.
...
## Arahan Claude untuk framework dan dokumentasi

- Baca source dan konfigurasi lokal sebelum mengandalkan pengetahuan umum
- Untuk React, TypeScript, Vite, Tailwind, Vitest, Radix UI, atau API library yang dapat berubah, gunakan Context7 atau dokumentasi resmi versi aktif
- Perlakukan contoh Next.js, Laravel, atau framework lain sebagai panduan consumer, bukan arsitektur repo
- Jangan mengubah showcase Vite menjadi framework lain ketika diminta memperbaiki kompatibilitas consumer
- Saat memperbarui API publik, sinkronkan `src/index.ts`, `package.json#exports`, tipe, test, showcase, dan dokumen consumer
- Saat menulis dokumentasi, verifikasi setiap command, path, versi, dan snippet terhadap working copy
- Gunakan `DESIGN.md` root untuk aturan aktif dan tandai informasi historis bila tetap perlu disebut
- Jangan menyatakan test, coverage, lint, build, atau audit lulus tanpa menjalankan perintah terkait
- Bila dokumentasi dan source berkonflik, ikuti urutan sumber kebenaran lalu sebutkan konflik pada handoff

## Batas tindakan

- Pertahankan perubahan pengguna lain dan batasi edit pada scope permintaan
