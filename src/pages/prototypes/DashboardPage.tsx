...
export function DashboardPage() {
  ...
  const columns = [
    ...
    { key: "population", label: "Populasi (Jiwa)", render: (v: string) => <span className="numeric">{v}</span> },
    { key: "growth", label: "Pertumbuhan", render: (v: string) => <span className="text-success numeric">{v}</span> },
    { key: "status", label: "Status Validasi", render: (v: string) => <StatusBadge variant={v as any}>{v.toUpperCase()}</StatusBadge> }
  ];
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Ringkasan indikator demografi Provinsi Kalimantan Utara.</p>
        </div>
        <Button>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <Card className="border-l-4 border-l-amber-500">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <Badge variant="outline" className="bg-amber-100 text-amber-800 border-amber-200">Sementara</Badge>
          </CardHeader>
          <CardContent>
