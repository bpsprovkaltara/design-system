    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <ShowcaseSection title="Varian Default">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          />
          <PerformanceCard title="Indeks Pembangunan" value={72.5} target={75} icon={Gauge} />
        </div>
        <CodeBlock>
          {`<PerformanceCard
  title="Total Penduduk"
...
/>`}
      ...
      <ShowcaseSection title="Varian Glass (overlay di backdrop berwarna)">
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-primary via-primary/80 to-amber-500 p-8">
          <div className="grid gap-4 md:grid-cols-3">
            <PerformanceCard variant="glass" title="Survei Aktif" value={42} icon={Activity} />
            <PerformanceCard
              variant="glass"
  ...
  />
      ...
      <ShowcaseSection title="Bento Grid Layout (data-density)">
        <div className="grid grid-cols-12 gap-4">
          ...
          <PerformanceCard
            ...
            icon={Users}
          />
          <PerformanceCard className="col-span-4" title="IPM" value={72.5} target={75} />
          <PerformanceCard
      ...
      <ShowcaseSection title="Loading State">
        <div className="grid gap-4 md:grid-cols-3">
          <PerformanceCard title="Memuat" value={0} loading />
          <PerformanceCard title="Memuat" value={0} variant="gradient" loading />
          <PerformanceCard title="Memuat" value={0} loading />
        </div>
        <CodeBlock>{`<PerformanceCard title="Memuat" value={0} loading />`}</CodeBlock>
      </ShowcaseSection>
    </div>
  )
}
