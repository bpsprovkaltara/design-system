...
import { BpsCombobox } from '@/components/ui/bps-combobox';
import { BpsDatePicker } from '@/components/ui/bps-date-picker';
      ...
      <ShowcaseSection title="Advanced Filters (Phase 6)">
        <div className="grid md:grid-cols-2 gap-6 border rounded-lg p-8 bg-card mb-6">
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">Gunakan untuk list yang panjang dengan kemampuan pencarian.</p>
          </div>
          <div className="space-y-2">
            <label className="label">Periode Rilis (Date Picker)</label>
            <BpsDatePicker placeholder="Pilih tanggal rilis" />
            <p className="text-xs text-muted-foreground mt-1">Terintegrasi dengan date-fns locale Indonesia.</p>
          </div>
        </div>
        <CodeBlock>
          {`<BpsCombobox options={[{value: "tarakan", label: "Kota Tarakan"}]} />
<BpsDatePicker onChange={(date) => console.log(date)} />`}
        </CodeBlock>
      </ShowcaseSection>
      ...
      <ShowcaseSection title="Text Inputs & Standard Select">
        <div className="grid md:grid-cols-2 gap-6 border rounded-lg p-8 bg-card">
          <div className="space-y-2">
            <label className="label">Nama Subjek <span className="text-destructive">*</span></label>
            <Input placeholder="Contoh: Kependudukan" />
          </div>
          <div className="space-y-2">
            <label className="label">Tahun Referensi</label>
            <Input type="number" defaultValue="2024" className="numeric border-destructive focus-visible:ring-destructive" />
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground mt-1">Tanpa kemampuan pencarian, cocok untuk list pendek ({"<"} 10 item).</p>
          </div>
        </div>
      </ShowcaseSection>
    </div>
