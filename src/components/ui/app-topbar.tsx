...
export function AppTopbar({ appTitle, start, end, className, children, ...props }: AppTopbarProps) {
  return (
    <header
      data-slot="app-topbar"
      className={cn(
        'flex h-topbar shrink-0 items-center gap-3 border-b border-border bg-card px-4 sm:px-6 lg:px-8',
        className
      )}
      {...props}
    >
